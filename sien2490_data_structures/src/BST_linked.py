...
class BST:
    ...
    def _remove_aux(self, node, key):
        ...
        if node is None:
            ...
            if node._right is None is None and node._left:
                node= None
                ...
                r= self._delete_node_left(node)
                
                if r is not None:
                    self._remove_aux(node, r._value)
                    r._left= node._left
                    r._right= node._right
                    node= r
    ...
    def _is_balanced_aux(self, node):
        ...
        if node._height== 1:
            ...
            #
            if node._left is not None:
                balanced, height1= self._is_balanced_aux(node._left)
                
            if node._right is not None:
                balanced, height2= self._is_balanced_aux(node._right)
                #
            if height1!= height2:
            ...
            height= 1+(height if height1> height2 else height2)
