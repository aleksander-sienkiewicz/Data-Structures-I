...
class Hash_Set:
    ...
    def insert(self, value):
        ...
        inserted= False
        crpt= hash(value)
        sl= crpt% self._capacity
        #Inserts value into the Hash Set, allows only one copy of value.
        
        if self._count== 0:
            self._table[sl].append(value)
            inserted= True
            
        else:
            if self._table[sl].is_empty():
                self._table[sl].append(value)
                inserted= True
                
            else:
                j= self._table[sl].index(value)
                if j== -1:
    ...
    def remove(self, key):
        ...
        if self._count== 0:
            ...
            if self._table[sl].is_empty():
                ...
                j= self._table[sl].index(key)
                if j== -1:
                    value= None
                    
                else:
                    value = self._table[sl][j]
                    self._table[sl].pop(j)
                    self._count-=1
                    #
        return(value)
    ...
    def _rehash(self):
        ...
        for i in self._table:
            for j in i:
                crpt= hash(j)% self._capacity
                new_table[crpt].append(j)
                #
