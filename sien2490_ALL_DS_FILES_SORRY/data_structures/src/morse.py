...
class ByLetter:
    ...
    def __eq__(self, target):
        ...
        return self.letter.lower() == target.letter.lower()
...
def fill_letter_bst(bst, values):
    ...
    for element in values:
        bst.insert(ByLetter(element[0],element[1]))
    return
...
def fill_code_bst(bst, values):
    ...
    """
    for element in values:
        bst.insert(ByCode(element[0],element[1]))
    return
...
def encode_morse(bst, text):
    ...
    for char in text:
        if char.isalpha():
            element = bst.retrieve(ByLetter(char,None))
            result += element.code
            result += " "
        elif char == " ":
            result += '\n'
...
def decode_morse(bst, code):
    ...
    for element in letters:
        result += bst.retrieve(ByCode(None,element)).letter
